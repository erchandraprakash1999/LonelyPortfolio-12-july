      ...
      <div className=" mx-auto  bg-[linear-gradient(#e5e7eb,white)] font-ChakraPetch">
        <div className="container mx-auto px-4 py-6">
          <div className="">
            ...
            <p className="text-gray-600 mt-4 text-xl" dangerouslySetInnerHTML={{ __html: data?.description }}></p>

          </div>
          <div className=" grid grid-cols-1 md:grid-cols-2 gap-6">
            {data?.items.map(({ name, value }, index) => {
              return <AnimatedProgressBar k={name} val={value} key={index} theme={theme} />
            })}
          </div>
        </div>
      </div>
